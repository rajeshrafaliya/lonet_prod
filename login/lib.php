...
function core_login_process_password_reset_request() {
    ...
    echo $OUTPUT->header();
	//Added by Nitesh
	echo '<h5 class="forgettitle" style="color: #374151;text-align: center;font-size: 32px;font-weight: 700;line-height: 40px;letter-spacing: -0.64px;">Forgot your password</h5>';
	echo '<p class="my-0 forgetinst" style="color: #374151;text-align: center;font-size: 18px;font-weight: 400;line-height: 24px;">To reset your password, submit your username or your email address below.</p>';
	echo '<p class="my-0" style="color: #374151;text-align: center;font-size: 18px;font-weight: 400;line-height: 24px;">If we can find you in the database, an email will be sent to your email address, with instructions how to get access again.</p>';
    // echo $OUTPUT->box(get_string('passwordforgotteninstructions2'), 'generalbox boxwidthnormal boxaligncenter');
	echo '<div class="forgetform" style="width: 100%;display: flex;align-items: center;justify-content: center;margin-top:64px;">';
    $mform->display();
	echo '</div>';
	echo '<div class="suboverrideimg contactsuboverride" style="margin-top:-140px;width:auto;right:0;">
<img src="/theme/lonet/pix/suboverride.png">
</div>';
    echo $OUTPUT->footer();
}
...
function core_login_process_password_set($token) {
    ...
    if (empty($data)) {
        ...
        $PAGE->verify_https_required();
        echo $OUTPUT->header();
		echo '<h5 class="forgettitle" style="color: #374151;text-align: center;font-size: 32px;font-weight: 700;line-height: 40px;letter-spacing: -0.64px;">Change your password</h5>';
		echo '<p class="my-0 forgetinst" style="color: #374151;text-align: center;font-size: 18px;font-weight: 400;line-height: 24px;">Keep your account secure with a strong, updated password.</p>';
        // echo $OUTPUT->box(get_string('setpasswordinstructions'), 'generalbox boxwidthnormal boxaligncenter');
		echo '<div class="forgetform" style="width: 100%;display: flex;align-items: center;justify-content: center;margin-top:64px;">';
        $mform->display();
		echo '</div>';
		echo '<div class="suboverrideimg" style="margin-top:-200px;right:100px;z-index:1;">
