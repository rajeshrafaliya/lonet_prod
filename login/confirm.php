    ...
    if ($confirmed == AUTH_CONFIRM_ALREADY) {
        ...
        if (!$user->suspended) {
            ...
            if ($user->profile['role'] == 'Teacher') {
                sendMail($user, 'teacherwelcome', ['firstname' => $user->firstname]);
            }
        }
        ...
        echo $OUTPUT->single_button($profile_link, get_string('continue'));//core_login_get_return_url()
        sendMail($user, 'learnerwelcome', ['firstname' => $user->firstname]);
        echo $OUTPUT->box_end();
        echo $OUTPUT->footer();
        exit;
    } else {
