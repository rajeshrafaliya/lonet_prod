    ...
    public static function prepare_identity_providers_for_output($identityproviders, renderer_base $output) {
        ...
        foreach ($identityproviders as $idp) {
			...
			if (!empty($idp['iconurl'])) {
				if($idp['name'] == 'Google'){
					$idp['iconurl'] = $CFG->wwwroot.'/theme/lonet/pix/auth/google.svg';
				}elseif($idp['name'] == 'LinkedIn' || $idp['name'] == 'Linkedin'){
					$idp['iconurl'] = $CFG->wwwroot.'/theme/lonet/pix/auth/linkedin.svg';
				}elseif($idp['name'] == 'facebook'){
					$idp['iconurl'] = $CFG->wwwroot.'/theme/lonet/pix/auth/facebook.svg';
				}else{	
					$idp['iconurl'] = $idp['iconurl']->out(false);
				}
			}
...
function signup_validate_data($data, $files) {
    ...
    if ($DB->record_exists('user', array('firstname' => $data['firstname'], 'lastname' => $data['lastname'], 'mnethostid' => $CFG->mnet_localhost_id))) {
        $errors['firstname'] = get_string('firstnameexists');
        $errors['lastname'] = get_string('lastnameexists');
    }    
    ...
    if ($DB->record_exists('user', array('username' => $data['username'], 'mnethostid' => $CFG->mnet_localhost_id))) {
        ...
        if (!get_config('extendedusernamechars') && $data['username'] !== core_text::strtolower($data['username'])) {
