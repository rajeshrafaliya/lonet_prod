...
function user_not_fully_set_up($user, $strict = true) {
    ...
    if (empty($user->firstname) or empty($user->email) or over_bounce_threshold($user)) {
        return true;
    }
...
function email_to_user($user, $from, $subject, $messagetext, $messagehtml = '', $attachment = '', $attachname = '',
	...
	if($subject == get_string('email_lessonstatusrequeststudent_subject','local_lonet')){ //Added by Nitesh
		$mail->AddBCC('lonet.academy+3b1b4aafee@invite.trustpilot.com');
	}
    if ($mail->send()) {
        set_send_count($user);
        if (!empty($mail->SMTPDebug)) {
