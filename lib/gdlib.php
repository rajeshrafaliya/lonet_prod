...
/********************Added by Nitesh**************************/
function process_new_icon_userpic($context, $component, $filearea, $itemid, $originalfile, $preferpng = false) {
    global $CFG;
    ...
    $icon['filename'] = 'original'.$imageext;
    $fileoriginal = $fs->create_file_from_string($icon, $originaldata);

    // return [
        // 'icon1' => $file1->get_id(),
        // 'icon2' => $fs->get_file($context->id, $component, $filearea, $itemid, '/', 'f2'.$imageext)->get_id(),
        // 'icon3' => $fs->get_file($context->id, $component, $filearea, $itemid, '/', 'f3'.$imageext)->get_id(),
        // 'original' => $fileoriginal->get_id()
    // ];    
	return $file1->get_id();
}
