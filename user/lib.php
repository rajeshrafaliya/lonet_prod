...
function user_create_user($user, $updatepassword = true, $triggerevent = true) {
    ...
    // Check username.
    if (!get_config('extendedusernamechars') && $user->username !== core_text::strtolower($user->username)) {
    ...
    if (!empty($SESSION->referral_code)) {
        $user->url = $SESSION->referral_code;
    }
    ...
    // user::addUserToMailingList($newuserid);
    if (!empty($SESSION->referral_code)) {
        $user->id = $newuserid;
        user::addReferralReward($user, $user->url);
    }
...
function user_update_user($user, $updatepassword = true, $triggerevent = true) {
	...
	//added by Nitesh
	if($DB->record_exists_sql("SELECT * FROM {user} WHERE username='".$user->username."'")){
		unset($user->username);
	}
...
function user_get_user_navigation_info($user, $page, $options = array()) {
	...
	*/
	...
	if($user->profile['role'] == 'Learner'){
		...
		$myprofile->pix = "a/graduation";
		$returnobject->navitems[] = $myprofile;
	}

    // Links: Teacher Profile.
    $teacherprofile = new stdClass();
    $teacherprofile->itemtype = 'link';
    $teacherprofile->url = new moodle_url('/teacher/'.$user->id);
    $teacherprofile->title = get_string('teacherprofile');
    $teacherprofile->titleidentifier = 'teacherprofile,moodle';
    $teacherprofile->pix = "a/owl";

    //START RAJESH 18-04-2022
    if($user->profile['role'] == "Teacher"){
        $returnobject->navitems[] = $teacherprofile;    
	}
    //END RAJESH 18-04-2022
