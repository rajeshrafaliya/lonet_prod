...
if ($usernew = $userform->get_data()) {
    ...
    if ($returnto === 'profile') {
        if ($course->id != SITEID) {
			...
			if($usernew->profile_field_role == 'Learner'){
				$returnurl = new moodle_url('/user/profile.php', array('id' => $user->id));
			}else{
				$returnurl = new moodle_url('/teacher/'.$user->id.'', array());
			}
        }
    } else {
...
if(!empty($user->profile_field_tutorrole)){
	echo '<style>#id_category_2 .fitem:not(.fitem_fadvcheckbox){display:block;}</style>';
}
if(!empty($user->profile_field_coachrole)){
	echo '<style>#id_category_5 .fitem:not(.fitem_fadvcheckbox){display:block;}</style>';
}
if(!empty($user->profile_field_trainerrole)){
	echo '<style>#id_category_6 .fitem:not(.fitem_fadvcheckbox){display:block;}</style>';
}
?>
<script>
$(document).ready(function(e) {
	$('#id_profile_field_tutorrole').change(function(){
		if($(this).is(':checked')){
			$('#id_category_2 .fitem:not(.fitem_fadvcheckbox)').show();
		} else {
			$('#id_category_2 .fitem:not(.fitem_fadvcheckbox)').hide();
		}
	});	
	$('#id_profile_field_coachrole').change(function(){
		if($(this).is(':checked')){
			$('#id_category_5 .fitem:not(.fitem_fadvcheckbox)').show();
		} else {
			$('#id_category_5 .fitem:not(.fitem_fadvcheckbox)').hide();
		}
	});	
	$('#id_profile_field_trainerrole').change(function(){
		if($(this).is(':checked')){
			$('#id_category_6 .fitem:not(.fitem_fadvcheckbox)').show();
		} else {
			$('#id_category_6 .fitem:not(.fitem_fadvcheckbox)').hide();
		}
	});
	
	if($('#id_profile_field_tutorrole').is(':checked')){
		$('#id_category_2 .fitem:not(.fitem_fadvcheckbox)').show();
	}	
	if($('#id_profile_field_coachrole').is(':checked')){
		$('#id_category_5 .fitem:not(.fitem_fadvcheckbox)').show();
	}	
	if($('#id_profile_field_trainerrole').is(':checked')){
		$('#id_category_6 .fitem:not(.fitem_fadvcheckbox)').show();
	}
	
	if($('#id_profile_field_role').val() == 'Learner'){
		$('#id_category_2,#id_category_5,#id_category_6').hide();
		$('#fitem_id_profile_field_videourl,#fitem_id_profile_field_linkedinpage,#fitem_id_profile_field_useinmarketing,#fitem_id_profile_field_citizenship,#fitem_id_profile_field_iknumber,#fitem_id_profile_field_languagesnative,#fitem_id_profile_field_teachingexperience,#fitem_id_profile_field_youngestage,#fitem_id_profile_field_onlinetools').hide();
	...
	$('#id_profile_field_role').change(function(){
		if($('#id_profile_field_role').val() == 'Learner'){
			...
			$('#id_category_2,#id_category_5,#id_category_6').show();
			$('#fitem_id_profile_field_videourl,#fitem_id_profile_field_linkedinpage,#fitem_id_profile_field_useinmarketing,#fitem_id_profile_field_citizenship,#fitem_id_profile_field_iknumber,#fitem_id_profile_field_languagesnative,#fitem_id_profile_field_teachingexperience,#fitem_id_profile_field_youngestage,#fitem_id_profile_field_onlinetools').show();
		}
	});
});
</script>
