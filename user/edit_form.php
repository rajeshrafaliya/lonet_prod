...
class user_edit_form extends moodleform {
    ...
    public function definition () {
		...
		$mform->setExpanded('moodle_interests');
...
/* 		if(!isset($usernew->profile_field_languagesnative)){
			$errors['profile_field_languagesnative'] = 'Please choose native languages.';
		} */
		...
		if(isset($usernew->profile_field_bestatteaching) && count($usernew->profile_field_bestatteaching) > 3){
			$errors['profile_field_bestatteaching'] = 'you can choose max 3.';
		}
		...
		if(isset($usernew->profile_field_languagestraining) && count($usernew->profile_field_languagestraining) > 1 && !empty($usernew->profile_field_trainerrole)){
			$errors['profile_field_languagestraining'] = 'you can choose max 1 language.';
		}
		if(!empty($usernew->profile_field_languagesnative) && !empty($usernew->profile_field_trainerrole)){
			$flag = false;
		...
		if(!empty($usernew->profile_field_tutorrole) && !empty($usernew->profile_field_coachrole) && !empty($usernew->profile_field_trainerrole)){
			$errors['profile_field_tutorrole'] = 'you can choose max 2 roles.';
		}
        // Next the customisable profile fields.
