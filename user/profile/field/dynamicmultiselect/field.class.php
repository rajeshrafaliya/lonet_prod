...
class profile_field_dynamicmultiselect extends profile_field_base {
    ...
    public function __construct($fieldid=0, $userid=0) {
        ...
        if ($fieldid !== 0 /*&& $userid !== 0*/) {
            ...
            // Set the data key.
            if ($this->data !== null) {
                $this->datatmp = ($this->data ? json_decode($this->data) : []);
    ...
    public function edit_field_set_default($mform) {
        if (false !== array_search($this->field->defaultdata, $this->options)) {
            $defaultkey = array_search($this->field->defaultdata, $this->options);
        } else {
            $defaultkey = '';
        }
    ...
    public function edit_load_user_data($user) {
        ...
        //$user->{$this->inputname} = array_values($this->data ? json_decode($this->data) : []);
    }
    ...
    public function display_data() {
        global $DB;
        $rs = $DB->get_records_sql($this->field->param1);
        $ret = '';
