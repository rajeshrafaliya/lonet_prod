	...
	if ($USER->id == $order->studentid || is_siteadmin()) {
		...
		?>
                ...
                <table style="width:100%;">
                    ...
                    <tbody>
                        ...
                        <?php foreach ($products as $product) {
							...
							}elseif($product->ispackage){
								$lesson = lesson::get_pkg_by_id($product->lessonid);
								$lprice = lesson::get_pkg_price($lesson);
							}else{
                            ...
                            <tr>
								...
								<?php } 
								if(isset($lesson->offer)){
									echo '<td>'.$lesson->name.'</td>';
								}else{
									echo '<td>'.$lesson->name.' with '. fullname($teacher, true).'</td>';
								}
								?>
            ...
            <p style="text-align:right;font-size:16px;">
                ...
                <?php } ?>
                <!-- RAJESH 14_12_2022 -->
                <strong>Total: </strong> &euro; <?= number_format($transaction->amount - $transaction->processing_fee, 2) ?>
                ...
                <?php if ($transaction->processing_fee > 0) { 
						if($transaction->method == 'card'){
							$text = 'Card Payment Commission';
						}else{
							$text = 'PayPal Payment Commission';
						}
				?>
                <br><strong><?= $text ?>: </strong>+ &euro; <?= number_format($transaction->processing_fee, 2) ?>
                <?php } ?>
                <?php if ($transaction->processing_fee > 0 || $transaction->used_balance_amount) { ?>
                <br><strong>Paid: </strong> &euro; <?= number_format($transaction->amount - $transaction->used_balance_amount, 2) ?>
                <?php } ?>
            </p>
            <?php if ($products) { ?>
                <br>
        ...
        <?php } else { ?>
			...
			if($transaction->method === 'reward'){
				if(!empty($transaction->data)){
					$decodedata = json_decode($transaction->data);
					if(!empty($decodedata->lessonid)){
						$dealdata = $DB->get_record_sql("SELECT * FROM {lonet_lesson} WHERE id=".$decodedata->lessonid."");
						echo $dealdata->name.' reward ' . $transaction->description;
					}else{
						echo 'Referral reward ' . $transaction->description;
					}
				}else{
					echo 'Referral reward ' . $transaction->description;
				}
			}else{
				echo 'Addition to Wallet';
			}				
			?>
			</h2>
            <p style="text-align:right;font-size:16px;">
