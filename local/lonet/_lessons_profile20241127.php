...
if ($student_lessons || $teacher_lessons) { ?>
	...
	<!-- profile container -->
	<div class="row user-row" style="background:none;"><div class="col-xs-12">
	<!-- profile container -->
	...
	<?php } else { ?>
		<?php if ($role == 'teacher' && $teacher_lessons) { ?>
			<h3 style="color: #374151;text-align: center;font-size: 24px;font-weight: 700;"><?= get_string('pendingrequests', 'local_lonet') ?></h3>
			<div id="teaching" class="container-fluid">
				<?php 
					$i = 0;
					foreach ($teacher_lessons as $lesson) {
						if($lesson->status > 0){continue;}
						echo order_product::get_lesson_html_profile($lesson);
						$i++;
					} 
					if(empty($i)){ echo '<p class="text-center">There is no pending requests.</p>';}
				?>
			</div>
		<?php } elseif ($role == 'learner' && $student_lessons) { ?>
			<h3 style="color: #374151;text-align: center;font-size: 24px;font-weight: 700;"><?= get_string('pendinglessons', 'local_lonet') ?></h3>
