...
if ($price = order::get_price()) {
    ...
    $order_amount = $transaction->amount - $used_balance;

    if ($method === 'paypal') {
        $amount_base = $transaction->amount - $transaction->processing_fee;
        $processing_fee = round($order_amount * 0.07, 2);
		...
		$amount_base = $transaction->amount - $transaction->processing_fee;
        $processing_fee = round($order_amount * 0.03, 2);
        $amount = $amount_base + $processing_fee;
        if ($amount != $transaction->amount) {
            $DB->update_record('lonet_order_transaction', ['id' => $transaction->id, 'amount' => $amount, 'processing_fee' => $processing_fee]);
        }
    ...
    $transaction = order::get_transaction();
    $order_amount = $transaction->amount - $used_balance;
    /** START Àlex 2024-11-21 Add correct order amount to the payment */    
    if ($method === 'paypal') {
        //$result = order_transaction::call_setup($method);
        $payment->callPayPalApi($order_amount, $transaction->orderid, $method);
    } else if ($method === 'card') {
        $payment->callPaymentApi($order_amount, $transaction->orderid, $method);
    }
    /** END Àlex 2024-11-21 Add correct order amount to the payment */
