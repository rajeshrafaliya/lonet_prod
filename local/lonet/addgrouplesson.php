...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['groupaction']) && isset($_POST['grouplesson'])) {
	...
	if(!empty($_FILES["fileToUpload"]["name"]) && !empty($_FILES["fileToUpload"]["tmp_name"])){
		$target_dir = $CFG->dirroot."/local/lonet/thumbnail/";
		$path_parts = pathinfo($_FILES["fileToUpload"]["name"]);
		$target_file = $target_dir . $path_parts['filename'].'_'.time().'.'.$path_parts['extension'];
		$modified_filename =  basename($target_file);
		$imageFileType = strtolower(pathinfo($target_file,PATHINFO_EXTENSION));
		$check = getimagesize($_FILES["fileToUpload"]["tmp_name"]);
		if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
			$glessonrecord = $DB->get_record_sql("SELECT * FROM {lonet_group_lessons} WHERE id=".$getlessonid."");
			$imagePath = $CFG->dirroot.'/local/lonet/thumbnail/'.$glessonrecord->thumbnail;
			unlink($imagePath);
			$DB->set_field( 'lonet_group_lessons', 'thumbnail', $modified_filename, array( 'id' => $getlessonid ) );
		} else {
	...
	redirect(new \moodle_url('/local/lonet/lessons.php?tab=grouplessons&teacherid='.$_POST['teacherid']), 'Group lesson added successfully', 10,\core\output\notification::NOTIFY_SUCCESS);
	// echo $result;
	// exit();
}
...
<?php
	foreach (schedule::get_blocked_days($teacherid) as $schedule) {
		...
		$lmaxamount = '';
		$lminamount = '';
		$lessondate = (new DateTime('now', core_date::get_user_timezone_object()))->setTimestamp(time())->format('Y-m-d');
		$lesson_desc = '';
		$whatlearn = '';
	if(!empty($lid)){
...
<h2 style="margin-bottom: 64px;color: #374151;text-align: center;font-size: 32px;font-weight: 700;line-height: 40px;"><?= get_string('addgrouplesson','local_lonet') ?></h2>
<div style="display:flex;justify-content:center;">
<form action="" method="POST" id="grouplessonform" class="grouplessonform" enctype="multipart/form-data">
...
<input type="hidden" name="grouplesson[teacherid]" value="<?= $teacherid ?>">	
  <div class="form-grouplesson">
   ...
   <input type="text" name="grouplesson[lessonname]" id="lessonname" value="<?= $lessonname ?>" placeholder="Add your lesson name">
  </div>
  <div class="form-grouplesson">
  ...
  <div class="form-grouplesson">
	...
	<input type="number" name="grouplesson[maxamount]" id="maxlearners" value="<?= $lmaxamount ?>">
  </div>
   <div class="form-grouplesson">
    <label for="maxamount"><?= get_string('minamountstudent', 'local_lonet') ?></label>
	<input type="number" name="grouplesson[minamount]" id="minlearners" value="<?= $lminamount ?>">
  </div>
  <div class="form-grouplesson">
  ...
  <div class="form-grouplesson">
   <label for="fileToUpload">Add a thumbnail</label>
    <div class="preview mr-2" id="imagePreview">
	<?php if(empty($lid)){ 
        echo '<p>No image uploaded</p>';
	}else{
		echo '<img src="'.$CFG->wwwroot.'/local/lonet/thumbnail/'.$lessondata->thumbnail.'">';
	}
	?>
    </div>
   <input type="file" name="fileToUpload" id="fileToUpload" accept="image/*">
  </div>  
  <div class="form-grouplesson" style="justify-content: center;">
    <button type="button" class="btn btn-success btn-action-group" data-action="groupsave"><?= get_string('savechanges') ?></button>
  </div>
</form>
</div>
<a class="btn btn-success bcktoteacher" href="/teacher/<?= $teacherid ?>"><span class="fa fa-angle-left"></span>&nbsp;&nbsp;<?= get_string('backtoprofile', 'local_lonet') ?></a>
</div>
<br>
<style>
#region-main{
...
.grouplessonform input,.grouplessonform select{
...
.grouplessonform .btn-action-group{
	border-radius: 60px;
...
.grouplessonform #imagePreview img{width:200px;height:200px;}
.grouplessonform #lessonname{width:430px;}
.grouplessonform #language{width: 340px;}
.grouplessonform #priceperperson,.grouplessonform #maxlearners,.grouplessonform #minlearners{width: 100px;}
</style>
<?= $OUTPUT->footer() ?>
