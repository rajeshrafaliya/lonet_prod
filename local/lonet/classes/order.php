	...
	public static function set_lesson_prices() {
		...
		if ($booking['promo_code']['id'] && $promotion = promo_code::get_by_id($booking['promo_code']['id'])) {
			...
			foreach ($lessons as $i => $lesson) {
				...
				if($lesson['offer'] > 0){
					$lessons[$i]['price'] = $DB->get_record('lonet_lesson', ['id' => $lesson['lessonid']])->price;
					$lessons[$i]['commission'] = 0;
					$lessons[$i]['payoutamount'] = 0;
				}else{
					$lessons[$i]['price'] = $price;
		...
		if(!empty($booking['order_products'])){
			foreach ($booking['order_products'] as $order_product) {
				if (!empty($order_product['is_confirmed'])) {
					...
					if($order_product['offer']){
						if (isset($order_product['studentid']) && $student = $DB->get_record('user', ['id' => $order_product['studentid']])) {
							$order_data = new \stdClass();
							$order_data->studentid = $order_product['studentid'];
							...
							if ($order_id = $DB->insert_record('lonet_order', $order_data)) {
								$product = $DB->get_record_sql("SELECT * FROM {lonet_order_product} WHERE orderid=".$booking['order_id']." and lessonid=".$order_product['lessonid']." AND studentid=".$order_product['studentid']."");
								$lessonreward = $DB->get_record_sql("SELECT * FROM {lonet_lesson} WHERE id=".$order_product['lessonid']."");
								$rewardamount = ($lessonreward->length) ? $lessonreward->length : 0;
								$order_product['rewardamount'] = $rewardamount;
								$rewardtransaction_data = new \stdClass();
								$rewardtransaction_data->reference = $product->id;
								$rewardtransaction_data->amount = $rewardamount;
								$rewardtransaction_data->method = 'reward';
								$rewardtransaction_data->orderid = $order_id;
                                $rewardtransaction_data->attempt = 1;
                                $rewardtransaction_data->userid = $order_product['studentid'];
                                $rewardtransaction_data->isincoming = 1;
                                $rewardtransaction_data->data = json_encode($order_product);
                                $rewardtransaction_data->iscompleted = 1;
                                $rewardtransaction_data->createdat = time();
                                $DB->insert_record('lonet_order_transaction', $rewardtransaction_data);
							}
						}
					}
				}
