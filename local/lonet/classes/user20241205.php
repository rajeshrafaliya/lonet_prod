...
class user {
	...
	public static function get_balance($userid) {
		...
		';
		$earnedd = $DB->get_record_sql($sql_11);
    
    // just a check what should happen GAVIN
    //get deal amount
//		$dealbalancearr = [];
...
//		$dealbalance = (!empty($dealbalancearr)) ? array_sum($dealbalancearr) : 0;
    // just a check what should happen GAVIN
		...
		if(is_siteadmin()){
      //Rajesh
//		 echo "<br>Rajesh = DealBalance:".$dealbalance." + IncomingBalance:".$incoming->balance. ' - UserBalance:'. $used->balance. ' + EarnedBalance:'. $earnedd->balance.' + Giftcard:'.$giftcard."=END<br>";
		}
	...
	public static function get_available_balance($userid) {
      ...
      //Rajesh
//      echo "Please beware that only site admins can see these messages.";
//      echo "<br>Haresh = DealBalance:".$dealbalance .' + IncomingBalance:'.$incoming->balance. 'R - UsedAndPendingBalance:'. $used_and_pending->balance. 'V + EarnedBalance:'. $earned->balance. 'S-'.teacher::get_pending_payout_request_amount($userid)."=END";
    }
//    return $dealbalance + $incoming->balance - $used_and_pending->balance + $earned->balance - teacher::get_pending_payout_request_amount($userid) + $giftcard;
		return $incoming->balance - $used_and_pending->balance + $earned->balance - teacher::get_pending_payout_request_amount($userid) + $giftcard;
	}
