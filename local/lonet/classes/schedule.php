...
class schedule {
	...
	public static function get_blocked_times_recurring($teacherid) {
        global $DB;
		$result = [];
		$records = $DB->get_records_sql("SELECT * FROM {lonet_schedule} WHERE teacherid=".$teacherid." AND isrecurring=1");
        $userTimezone = core_date::get_user_timezone_object();
		foreach ($records as $record) { //setting time labels
			$record->timefrom = (new DateTime('now', $userTimezone))->setTimestamp($record->timefrom)->format('H:i:s');
			$record->timeto = (new DateTime('now', $userTimezone))->setTimestamp($record->timeto)->format('H:i:s');
...
public static function get_schedule($teacherid, $week = 0, $edit = false) {
	...
	$recurring_blocks = $DB->get_records_sql("SELECT * FROM {lonet_schedule} WHERE isrecurring=1 AND teacherid=".$teacherid."");
	...
	for ($i = 0; $i < 24; $i++) {
		...
		foreach (['00', '30'] as $index => $minutes) {
			...
			// Existing block logic for specific times
			...
			foreach ($dates as $date => $label) {
				...
				if (in_array($date . ' ' . $time, $booked)) {
					...
					// Existing day and date block logic
				...
				// Add recurring block check here
if ($class == 'bg-success') { // Only block if still available
    $day = (new DateTime($date, $userTimezone))->format('N'); // Get the day number (1 = Monday, 7 = Sunday)
    $currentTimestamp = (new DateTime($date . ' ' . $time, $userTimezone))->getTimestamp(); // Get timestamp for the current time slot

    if (!empty($recurring_blocks)) {
        foreach ($recurring_blocks as $recurring_block) {
            $recurring_days = is_string($recurring_block->weekdays) ? json_decode($recurring_block->weekdays, true) : $recurring_block->weekdays;

            // Convert times to DateTime objects considering the correct timezones
            $recurring_timefrom = (new DateTime('now', $userTimezone))->setTimestamp($recurring_block->timefrom)->format('H:i:s');
            $recurring_timeto = (new DateTime('now', $userTimezone))->setTimestamp($recurring_block->timeto)->format('H:i:s');
            $current_time = (new DateTime('now', $userTimezone))->setTimestamp($currentTimestamp)->format('H:i:s');

            if (
                in_array($day, $recurring_days)
                && $current_time >= $recurring_timefrom
                && $current_time < $recurring_timeto
               // && $currentTimestamp <= $recurring_block->ends
            ) {
                $class = '';
                break;
            }
