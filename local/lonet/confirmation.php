	...
	<div class="row mx-0" style="display:flex;justify-content:space-between;padding:64px;">
            ...
            <?php if ($lessons) { ?>
                ...
                <?php foreach ($lessons as $index => $order_product) {
                    if (!empty($order_product['is_confirmed'])) {
						...
						}elseif($order_product->ispackage){
							$lesson = lesson::get_pkg_by_id($order_product->lessonid);
							$order_product->price = lesson::get_pkg_price($lesson);
							$lprice = lesson::get_booking_price_html_pkg($lesson, $order_product->price);
						}else{
		...
		<div class="col-sm-4 col-options">
			<div class="col-sm-12 user-row" style="padding: 32px 24px;border-radius: 40px;background:#EFEDE6;box-shadow: 0px 2px 6px 0px rgba(16, 24, 40, 0.06)">
				<h2><?= get_string('totalprice', 'local_lonet') ?> <span class="pull-right">&euro; <span class="price"><?= $transaction->amount - $transaction->processing_fee ?></span></span></h2><!--.00-->
