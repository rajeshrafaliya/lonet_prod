			...
			<div id="lesson-selection" class="col-sm-12" <?= ($has_language_selection && !$current_language ? 'style="display: none;"' : '') ?>>
				...
				<?php foreach ($grouplessons as $glesson) {
					if (!$glesson->istrial || $can_book_trial) {
						...
						$lessondate = ($glesson->timefrom ? (new DateTime('now', core_date::get_user_timezone_object()))->setTimestamp($glesson->timefrom)->format('D, M d') : null);
						...
						<span>Duration: <?= get_string('numminutes', 'core_moodle', max(30, (($lesson->length / 60) - 10))) ?></span></br>
						<span>Date: <?= $lessondate; ?></span></br>
						<span>Time: <?= $glessontimes->timefrom; ?></span></br>
						<span>Max attendees: <?=$glesson->maxamount;?></span></br>
						<span>Places left: <?= $glesson->maxamount-$glessonmembers;?></span>
						</label>
					<?php }
				} ?>
				</div>
