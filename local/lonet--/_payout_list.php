...
if ($lessons = teacher::get_lessons_for_payout($user->id)) {
	...
	if($payout_request && $payout_request->lessons){
		...
		foreach($ldecodes as $ldecode){
			if($DB->record_exists_sql('SELECT op.* FROM {lonet_order_product} op INNER JOIN {lonet_order} ord ON ord.id = op.orderid WHERE op.teacherid = ' . $user->id . '	AND op.lessonid = '.$ldecode.' AND ord.createdat > '. $sixmonth .'')){
				$saved_data[] = $ldecode;
			}
		}
	}
