...
class user {
	...
	public static function get_balance($userid) {
		...
		// echo "Incoming : ".$incoming->balance. '-used :'. $used->balance. '+ earn :'. $earned->balance.'+gift :'.$giftcard."=END";
	...
	public static function get_available_balance($userid) {
		...
		// echo "<br>in-".$incoming->balance. '-pending-'. $used_and_pending->balance. '+earn+'. $earned->balance. 'S-'.teacher::get_pending_payout_request_amount($userid)."=END";
    ...
    public static function getPublicDisplay($id) {
        ...
        if ($user) {
            $html .= $OUTPUT->user_picture($user, ['size' => '40']);
...
/* METADATA */
    ...
    public static function getCurrentPageTemplate() {
        ...
        $template = array_search($script_name, $template_urls);
        if ($template === false) {
