...
class order_product {
		...
		switch ($lesson->status) {
			...
			case self::STATUS_CONFIRMED:
				if ($lesson->starttime > time()) {
					$html .= '<button class="btn btn-danger btn-respond" data-action="cancel" data-role="' . $role . '">' . get_string('cancel', 'local_lonet') . '</button>';
					$html .= '<a target="_blank" href="'.$CFG->wwwroot.'/calendar/export_calendar.php?id='.$lesson->id.'" class="btn btn-info exportcal">Export for Google Calendar</a>';//Added by Nitesh
				} else {
					if (($is_teacher && !$lesson->teachercompleted) || ($is_learner && !$lesson->studentcompleted)) {
						$html .= '<button class="btn btn-success btn-respond" data-action="complete" data-role="' . $role . '">' . get_string('markcompleted', 'local_lonet') . '</button>';
						$html .= '<button class="btn btn-danger btn-respond" data-action="notcomplete" data-role="' . $role . '">' . get_string('marknotcompleted', 'local_lonet') . '</button>';
					}
				}
	...
	public static function get_lesson_for_payout_html($lesson, $checked = false) {
		...
		$html = '<tr data-id="' . $lesson->id . '">
			...
			<td>
				...
				<input type="checkbox" class="checkbox payout-confirmation-checkbox" id="confirm-' . $lesson->id . '" name="PayoutRequest[lessons][]" value="' . $lesson->id . '" ' . ($amount < 0 || $checked ? 'checked' : '') . ($amount < 0 ?  ' disabled' : '')  . '>
    ...
    public static function getLessonData(&$lessons) {
        ...
        foreach ($lessons as $lesson) {
            ...
            $data[] = [
                ...
                'status' => self::get_status_label($lesson),
                'actions' => self::get_buttons_html($lesson),
	...
	public static function get_status_label($lesson, $for_admin = false) {
		...
		return ($status ? get_string($status, 'local_lonet') : '') . $comment;
	}
