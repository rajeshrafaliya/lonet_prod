...
class order {
	...
	public static function set_lesson_prices() {
		...
		if ($booking['promo_code']['id'] && $promotion = promo_code::get_by_id($booking['promo_code']['id'])) {
			...
			foreach ($lessons as $i => $lesson) {
				...
				$lessons[$i]['price'] = lesson::get_price($DB->get_record('lonet_lesson', ['id' => $lesson['lessonid']]));
	...
	public static function complete_booking($transaction, $is_incoming = true) {
		...
		sendMail_booking($USER, $mail, ['fullname' => fullname($USER, true), 'reference' => $transaction->reference, 'link' => $transaction->orderid, 'yourproducts' => $yourproducts]);
